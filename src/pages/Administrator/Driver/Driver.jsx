...
const Driver = () => {
  ...
  const fetchDrivers = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8081/Mega_City_Cab_Service_App_war_exploded/person?type=DRIVER"
      );
  ...
  // Handle form submission to register a new driver
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      licenseType: formData.licenseType.trim() || "null", // Default to "null" if empty
    ...
    try {
      ...
      setNotification("Driver registered successfully!");
      setTimeout(() => {
        setNotification("");
      }, 3000);
      ...
      setNotification("Failed to register driver. Please try again.");
      setTimeout(() => {
        setNotification("");
      }, 3000);
    }
      ...
      <div className="flex flex-col gap-y-4 gap-8">
        <div className="grid sm:grid-cols-1 gap-x-4 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 grid-flow-row rounded-lg border border-slate-300 bg-white p-1 transition-colors dark:border-slate-700 dark:bg-slate-900">
          <div className="grid col-span-4 sm:col-span-4 p-2">
            <div className="flex items-center gap-2">
              ...
              <p className="card-title">Manage Driver Details</p>
            </div>
          </div>
          ...
          <div className="xl:col-span-2 col-span-4 flex flex-col gap-y-1 p-2">
            <p className="bookinputlable">License Number</p>
            <div className="bookinputfield">
              <input
                type="text"
                name="licenseNumber"
          ...
          {/* License Type Input */}
          <div className="xl:col-span-2 col-span-4 flex flex-col gap-y-1 p-2">
            <p className="bookinputlable">License Type</p>
            <div className="bookinputfield">
              <input
                type="text"
                name="licenseType"
                ...
                onChange={handleInputChange}
                placeholder="Enter License Type (e.g., light, heavy)"
                className="w-full bg-transparent text-slate-900 outline-0 placeholder:text-slate-300 dark:placeholder:text-slate-300/65 dark:text-slate-50"
              />
            </div>
          </div>
        ...
        <div className="card">
          <div className="card-header">
            <p className="card-title">Driver Details</p>
          </div>
              ...
              <table className="table">
                <thead className="table-header">
                  <tr className="table-row">
                    ...
                    <th className="table-head">License Number</th>
                    <th className="table-head">License Type</th>
                ...
                <tbody className="table-body">
                  {drivers.map((driver, index) => (
                    <tr key={driver.id} className="table-row">
                      <td className="table-cell">{driver.id}</td>
                      <td className="table-cell">{driver.name}</td>
                      <td className="table-cell">{driver.address}</td>
                      <td className="table-cell">{driver.mobile}</td>
